...
let create scope ({ clock; clear; start; finish; dir; value } : _ I.t) : _ O.t =
  ...
  let%hw_var key = Variable.reg spec ~width:14 in
  ...
  let wraps = Signal.wire 4 in
  let () = Signal.assign wraps
    (
      List.init 10 ~f:(fun i -> (i + 1) * 100)
      |> List.mapi ~f:(fun i thresh -> 
        (thresh, i + 1))
      |> List.fold ~init:(Signal.of_int_trunc ~width:4 0) ~f:(fun acc (thresh, count) ->
          mux2 (value >: Signal.of_int_trunc ~width:10 thresh)
                (Signal.of_int_trunc ~width:4 count)
                acc)
    )
  in
  ...
  let increment = Signal.wire 1 in
  let () = Signal.assign increment (
    mux2 (dir)
      (mux2 (loc.value <=: effective_value_8)
        (mux2 (loc.value ==: Signal.of_int_trunc ~width:8 0)
          (Signal.of_int_trunc ~width:1 0)
          (Signal.of_int_trunc ~width:1 1))
        (Signal.of_int_trunc ~width:1 0))
      (mux2 ((loc.value +: effective_value_8) >=: const_100)
        (Signal.of_int_trunc ~width:1 1)
        (Signal.of_int_trunc ~width:1 0))
    )
  in
    ...
    [ sm.switch
        ...
        ; ( Running
          , [ (* update location *)
            ...
            ;
              key <-- key.value +: (Signal.uresize (wraps +: Signal.uresize increment ~width:4) ~width:14)
